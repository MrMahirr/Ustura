...
export default function SalonRow({
  ...
  const mainRowFlex = 2.4 + 1.6 + 1.45 + 1.05 + 0.95;

  return (
    <View className="min-h-[88px] flex-row items-center px-6">
      <View
        className="min-h-[88px] min-w-0"
        style={{
          flex: mainRowFlex,
          flexDirection: 'row',
          alignItems: 'center',
        ...
        <Pressable
          ...
          className="min-h-[88px] min-w-0 flex-1 flex-row items-center"
          disabled={!onOpenDetail}
          style={({ hovered }) => [
            { minWidth: 0 },
            { backgroundColor: hovered && onOpenDetail ? adminTheme.cardBackgroundStrong : 'transparent' },
            Platform.OS === 'web'
              ? ({
      ...
      <View
        className="min-w-0 justify-center py-[18px]"
        style={[
          cellActionsStyle,
          { flexShrink: 0 },
          Platform.OS === 'web' ? ({ position: 'relative' as const, zIndex: 2 } as const) : null,
        ]}>
        <View className="flex-row items-center gap-1">
          {actions.map((action) => (
            <SalonActionIcon
